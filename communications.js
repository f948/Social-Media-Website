...
io.on('connection', function(socket) {
	...
	socket.on("checkPassword",function(passwordValue){
		...
		socket.emit("passwordRegistered",{username:usernameValue,isRegistered:registered});
	});
	...
	socket.on("getKeywordWebPages",function(keyword){
		...
		for(i=0;i<=webPages.length-1;i++){
			...
			for(j=0,k=0;j<=keyword.length-1 && k<=nameWithoutExtension.length-1;j++,k++){
				...
				else if(nameWithoutExtension[k] != keyword[j]){
					pointsAwarded--;
				}
		...
		for(i=0;i<=webPages.length-1;i++){
			...
			for(j=0,k=0;j<=splitKeyword.length-1 && k<=splitNameWithoutExtension.length-1;j++,k++){
				...
				else if(!splitNameWithoutExtension.includes(splitKeyword[j])){
					pointsAwarded--;
				}
