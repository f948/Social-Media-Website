...
var userData=[],webPages=[],points=[],thumbnailImages=[];
...
app.get('/:file',function(req,res){
	...
	if(connectedUsers<50){
		...
		if(req.params.file == "FILE WAS NOT SENT WITH THIS POST"){
			
			res.send("A file was not sent with this post");
		}
...
io.on('connection', function(socket) {
	...
	socket.on("createThumbnailImage",function(data){
		
		fs.writeFile(webPageId.toString()+"."+data.extension, data.content, function (err) {
				
		});
		
		thumbnailImages.push({image:'<img width="300" height="300" src='+webPageId.toString()+"."+data.extension+'>',name:webPageId.toString()+"."+data.extension,code:data.content});
		
		socket.emit("sendThumbnailImageSrc",webPageId.toString()+"."+data.extension);
		webPageId++;
	});
	...
	socket.on("doesFileExist",function(fileName){
		...
		for(i=0;i<=thumbnailImages.length-1;i++){
			
			if(thumbnailImages[i].name == fileName){
				fileExist=true;
	...
	socket.on("sendWebPage",function(data){
			...
			webPages.push({username:data.username,title:data.title,image:data.image,name:data.name,code:data.code,description:data.description,comments:"",views:0,date:data.date,id:webPageId,deleted:false});
			io.sockets.emit("getWebPage",{username:data.username,title:data.title,image:data.image,name:data.name,code:data.code,description:data.description,comments:"",views:0,date:data.date,id:webPageId,deleted:false});
			webPageId++;
