...
app.get('/:file',function(req,res){
	...
	if(connectedUsers<50){
		...
		if(req.params.file.includes("FILE WAS NOT SENT WITH THIS POST")){
...
io.on('connection', function(socket) {
	...
	socket.on("sendWebPage",function(data){
			...
			webPages.push({username:data.username,title:data.title,image:data.image,name:fileName,code:data.code,description:data.description,comments:"",views:0,date:data.date,id:webPageId,deleted:false});
			io.sockets.emit("getWebPage",{username:data.username,title:data.title,image:data.image,name:fileName,code:data.code,description:data.description,comments:"",views:0,date:data.date,id:webPageId,deleted:false});
			webPageId++;
