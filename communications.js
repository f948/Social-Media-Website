	...
	socket.on("addMessage",function(data){
		...
		webPageId++;
			...
			if(data.type == "message"){
				...
				messages.push({type:data.type,code:data.code,sender:data.sender,reciever:data.reciever});
					...
					if(!["jpg","jpeg","png","gif"].includes(data.name.split(".")[data.name.split(".").length-1])){
						...
						messages.push({type:data.type,name:webPageId.toString()+data.name,code:data.code,sender:data.sender,reciever:data.reciever});
