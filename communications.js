...
io.on('connection', function(socket) {
	...
	socket.on("sendWebPage",function(data){
			...
			if(data.name != "FILE WAS NOT SENT WITH THIS POST"){
				...
				data.name="FILE WAS NOT /SENT WITH THIS POST";
			}
			...
			webPages.push({profileImg:data.profileImg,username:data.username,title:data.title,image:data.image,imageSrc:data.imageSrc,name:data.name,code:data.code,description:data.description,comments:"",views:0,date:data.date,dateString:months[data.date.getMonth()]+" "+data.date.getDate().toString()+", "+data.date.getFullYear().toString()+" at "+hours+":"+data.date.getMinutes()+time,id:webPageId,deleted:false});
